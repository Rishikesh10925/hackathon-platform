...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        // Fetch events
        const eventRes = await axios.get('https://xe6yu454x0.execute-api.us-east-1.amazonaws.com/getEvents');
        const fetchedEvents = (eventRes.data && eventRes.data.events) ? eventRes.data.events : (Array.isArray(eventRes.data) ? eventRes.data : []);
        setEvents(fetchedEvents);

        // Fetch teams to cross-reference counts
        try {
          const teamRes = await axios.get('https://xe6yu454x0.execute-api.us-east-1.amazonaws.com/getTeams');
          if (teamRes.data && teamRes.data.teams) {
            setTeams(teamRes.data.teams);
          }
        } catch (teamErr) {
          console.warn("Could not fetch teams for stats:", teamErr);
        }

        // Fetch judges to cross-reference counts
        try {
          const judgeRes = await axios.get('https://xe6yu454x0.execute-api.us-east-1.amazonaws.com/getJudges');
          if (judgeRes.data && judgeRes.data.judges) {
            setJudges(judgeRes.data.judges);
          }
        } catch (judgeErr) {
          console.warn("Could not fetch judges for stats:", judgeErr);
        }
  ...
  const metricCards = [
    ...
    { title: "Active Teams", value: teams.length || 0, icon: <Users className="w-6 h-6 text-emerald-500" />, bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
    { title: "Registered Judges", value: judges.length || 0, icon: <Gavel className="w-6 h-6 text-purple-500" />, bg: "bg-purple-500/10", border: "border-purple-500/20" }
  ];
      ...
      <aside className="w-72 bg-slate-900 text-white flex flex-col shadow-2xl z-10">
        ...
        <nav className="flex-1 p-4">
          <ul className="space-y-2">
            ...
            <li>
              <button onClick={() => navigate('/manage-events')} className="w-full flex items-center gap-3 p-3 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {metricCards.map((m, i) => (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }} key={i} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-start justify-between relative overflow-hidden group">
                ...
                <div className={`p-4 rounded-xl ${m.bg} ${m.border} border relative z-10`}>
                  {m.icon}
                </div>
                <div className={`absolute -right-6 -bottom-6 w-24 h-24 ${m.bg} rounded-full blur-2xl opacity-50 group-hover:scale-150 transition-transform duration-500`} />
              </motion.div>
            ))}
          </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {events.map((event, i) => {
                      ...
                      const eventTeamsCount = teams.filter(t => t.EventID === eventId).length;
                      const eventJudgesCount = judges.filter(j => j.EventID === eventId).length;
                      return (
                      <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }} key={eventId || i} className="hover:bg-slate-50/80 transition-colors group">
                        <td className="py-4 px-6 font-mono text-sm font-medium text-slate-600">{eventId}</td>
                        <td className="py-4 px-6 font-semibold text-slate-900">{event.name || event.EventName}</td>
                        <td className="py-4 px-6 text-slate-600 flex items-center gap-2"><Users className="w-4 h-4 text-slate-400" /> {eventTeamsCount} Teams</td>
                        <td className="py-4 px-6 text-slate-600"><Gavel className="w-4 h-4 text-slate-400 inline mr-1" /> {eventJudgesCount} Judges</td>
                        ...
                        <td className="py-4 px-6 text-right">
                          <button onClick={() => navigate(`/leaderboard?eventId=${eventId}`)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
